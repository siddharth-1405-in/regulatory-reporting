...
def build_pdf(db: Session, instance_id: int) -> tuple[bytes, str]:
    """Returns (bytes, media_type) as a real PDF using fpdf2."""
    ...
    def _footer():
        ...
        pdf.cell(0, 5, f"Generated by the Agentic CAR Reporting Platform  ·  {inst.bank_name}  ·  {inst.period_label}  ·  Page {pdf.page_no()}", align="C")
    ...
    def _table_header(*cols_widths):
        ...
        for label, w in cols_widths:
            pdf.cell(w, 6, label.upper(), border="B", fill=True)
        pdf.ln()
    ...
    def _table_row(*cells_widths, bold: bool = False, alt: bool = False):
        ...
        for text, w, align in cells_widths:
            pdf.cell(w, 5.5, str(text or ""), border="B", align=align, fill=alt)
        pdf.ln()
    ...
    def _status_badge(text: str, good: bool):
        ...
        pdf.set_text_color(*WHITE)
        pdf.set_font("Helvetica", "B", 7)
        pdf.cell(28, 5, f"  {text}", fill=True, border=0)
        pdf.set_text_color(*DARK)
    ...
    pdf.set_text_color(*DARK)
    pdf.cell(0, 12, inst.bank_name, ln=True)

    pdf.set_font("Helvetica", "", 10)
    pdf.set_text_color(*MUTED)
    pdf.cell(0, 6, f"Capital Adequacy Return  ·  {inst.period_label}  ·  Period end {inst.period_end.strftime('%d %b %Y')}", ln=True)
    pdf.ln(2)
