...
export function NarrativeSection({ id }: { id: number }) {
  ...
  if (narrs.length === 0) return <Panel title="Narrative review"><Empty text="No narrative drafted yet — run the agents." /></Panel>;
  return (
    <div className="grid grid-cols-2 gap-4">
      {narrs.map((n) => (
        <Panel key={n.id} eyebrow={n.language === "ar" ? "العربية · Arabic" : "English"} title={`Executive narrative · v${n.version}`}
          actions={<Badge tone={n.status === "approved" ? "ok" : "warn"}>{n.status}</Badge>}>
          ...
          <div className="flex items-center justify-between mt-2.5">
            <span className="text-[10px] text-uq-muted">Drafted by Narrative agent · {pct(n.confidence, 0)} confidence</span>
            {n.status !== "approved" && <Button onClick={() => approve.mutate(n.id)} disabled={approve.isPending}>Approve</Button>}
          </div>
        </Panel>
      ))}
    </div>
